...
function twig_escape_filter(Twig_Environment $env, $string, $strategy = 'html', $charset = null, $autoescape = false)
{
    ...
    switch ($strategy) {
        case 'js':
            ...
            if (null === $string = preg_replace_callback('#[^\p{L}\p{N} ]#Su', '_twig_escape_js_callback', $string)) {
        ...
        case 'js':
            ...
            if (null === $string = preg_replace_callback('#[^a-zA-Z0-9]#Su', '_twig_escape_csss_callback', $string)) {
                throw new Twig_Error_Runtime('The string to escape is not a valid UTF-8 string.');
            }
...
function _twig_escape_js_callback($matches)
{
    ...
    // \uHHHH
    return '\\u'.substr('0000'.bin2hex(twig_convert_encoding($char, 'UTF-16BE', 'UTF-8')), -4);
}
