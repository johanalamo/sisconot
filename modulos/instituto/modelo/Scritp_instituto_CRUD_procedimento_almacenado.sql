...
CREATE OR REPLACE FUNCTION sis.f_instituto_seleccionar_por_codigo(refcursor,integer) -- analisa esta nomenclatura
  RETURNS refcursor AS
$BODY$
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION sis.f_instituto_seleccionar_por_codigo(refcursor,integer)
  OWNER TO postgres;


select sis.f_instituto_seleccionar_por_codigo('curso',9);
FETCH ALL IN "curso";
