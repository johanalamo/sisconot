...
COMMENT ON COLUMN sis.t_est_periodo.codigo is 'Código de la entrada.';
...
COMMENT ON COLUMN sis.t_periodo.fec_final is 'Fecha de fin del periodo académico.';
COMMENT ON COLUMN sis.t_periodo.observaciones is 'Observaciones del periodo académico.';
COMMENT ON COLUMN sis.t_periodo.cod_estado is 'Estado del periodo académico (Abierto o Cerrado en el momento de creación del script)';
...
create table sis.t_electiva (
	...
	CONSTRAINT cf_electiva__cur_estudiante PRIMARY KEY(cod_cur_estudiante)
		REFERENCES sis.t_cur_estudiante (codigo),
	CONSTRAINT cf_electiva__pensum PRIMARY KEY(cod_pensum)
		REFERENCES sis.t_pensum (codigo),
	CONSTRAINT cf_electiva__trayecto PRIMARY KEY(cod_trayecto)
...
create TABLE sis.t_acreditable(
	...
	CONSTRAINT cf_acreditable__trayecto FOREIGN KEY(cod_trayecto)
		REFERENCES sis.t_trayecto(codigo),
	CONSTRAINT chk_acreditacion_01 CHECK (uni_credito >= 0),
);
...
COMMENT ON COLUMN sis.t_convalidacion.cod_cod_trayecto is 'Código del trayecto en el que se tomará la convalidación.';
...
COMMENT ON COLUMN sis.t_convalidacion.descripcion is 'Descripción opcional sobre la convalidación';
